...
const uint8_t REG_IODIR[2][2]   = {{0x00, 0x00}, {0x01, 0x10}};
const uint8_t REG_IPOL[2][2]    = {{0x02, 0x01}, {0x03, 0x11}};
const uint8_t REG_GPINTEN[2][2] = {{0x04, 0x02}, {0x05, 0x12}};
const uint8_t REG_DEFVAL[2][2]  = {{0x06, 0x03}, {0x07, 0x13}};
const uint8_t REG_INTCON[2][2]  = {{0x08, 0x04}, {0x09, 0x14}};
const uint8_t REG_IOCON[2][2]   = {{0x0A, 0x05}, {0x0B, 0x15}};
const uint8_t REG_GPPU[2][2]    = {{0x0C, 0x06}, {0x0D, 0x16}};
const uint8_t REG_INTF[2][2]    = {{0x0E, 0x07}, {0x0F, 0x17}};
const uint8_t REG_INTCAP[2][2]  = {{0x10, 0x08}, {0x11, 0x18}};
const uint8_t REG_GPIO[2][2]    = {{0x12, 0x09}, {0x13, 0x19}};
const uint8_t REG_OLAT[2][2]    = {{0x14, 0x0A}, {0x15, 0x1A}};
...
MCP23017::MCP23017(uint8_t addr, CtrlI2C *ctrl)
: _bank(0)
, DeviceI2C(0x20 | (addr & 0x3), ctrl)
{
	GPIO_BIT gpio;
	uint8_t iocon = REG_IOCON[0][0];
	_twi->transfer(_address, &iocon, 1, &gpio.octet, 1);
...
MCP23017::~MCP23017()
{
	uint8_t bank[2]; bank[0] = REG_IOCON[0][_bank]; bank[1] = 0;
...
void MCP23017::init(uint8_t port, uint8_t dir, uint8_t pol, uint8_t pullup, uint8_t irq, uint8_t defval_pin, uint8_t defval)
{
	...
	uint8_t registre = REG_IOCON[port][_bank];
	_twi->transfer (_address, &registre, 1, &ctrl.octet, 1);
	...
	uint8_t iocon[2];
	iocon[0] = REG_IOCON[port][_bank];
	iocon[1] = ctrl.octet;
	_twi->set(_address, iocon, 2);

	// io dir
	uint8_t iodir[2];
	iodir[0] = REG_IODIR[port][_bank];
	iodir[1] = dir;
	...
	if (dir)
	{
		// ipol
		uint8_t ipol[2];
		ipol[0] = REG_IPOL[port][_bank];
		ipol[1] = pol;
		_twi->set(_address, ipol, 2);

		// pull-up resistor
		uint8_t gppu[2];
		gppu[0] = REG_GPPU[port][_bank];
		gppu[1] = pullup;
		...
		uint8_t gpinten[2];
		gpinten[0] = REG_GPINTEN[port][_bank];
		gpinten[1] = irq;
		...
		uint8_t intcon[2];
		intcon[0] = REG_INTCON[port][_bank];
		intcon[1] = defval_pin;
		...
		defval_reg[0] = REG_DEFVAL[port][_bank];
		defval_reg[1] = defval;
...
uint8_t MCP23017::getIrq(uint8_t port)
{
	...
	uint8_t value = 0;
	uint8_t intf = REG_INTF[port][_bank];
	_twi->transfer(_address, &intf, 1, &value, 1);
	
	// INTCAP
	// Reflects the logic level on the port pins at the time of interrupt due to pin change <7:0>
	// 1 = Logic-high
	// 0 = Logic-low
	uint8_t intcap = REG_INTCAP[port][_bank];
	_twi->transfer(_address, &intcap, 1, &value, 1);
...
{
	uint8_t value = 0;
	uint8_t cmd = REG_GPIO[port][_bank];
	_twi->transfer (_address, &cmd, 1, &value, 1);
